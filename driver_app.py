        ...
        if filter_df.empty:
            ...
            if not income_df.empty:
                ...
                fig_trend = px.bar(
                    daily_chart, 
                    x='Date', 
                    y=['Income', 'Expense'], # เทียบรายรับ vs รายจ่าย ตรงๆ
                    title="เปรียบเทียบ: รายรับ (เขียว) vs รายจ่าย (แดง)",
                    barmode='group', # วางข้างกันเพื่อให้เปรียบเทียบความสูงง่ายๆ
                    color_discrete_map={'Income': '#2ECC71', 'Expense': '#E74C3C'},
                    labels={'value': 'บาท', 'variable': 'รายการ', 'Date': 'วันที่'}
                )
                # เปลี่ยนชื่อใน Legend ให้เข้าใจง่าย
                new_names = {'Income': 'รายรับรวม', 'Expense': 'รายจ่ายรวม'}
                fig_trend.for_each_trace(lambda t: t.update(name = new_names[t.name]))
            ...
            if not income_df.empty:
                ...
                if not pivot_hm.empty:
                    fig_hm = px.imshow(
                        pivot_hm, labels=dict(x="ชั่วโมง", y="วัน", color="บาท"),
