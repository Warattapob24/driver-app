...
# ==========================================
# TAB 3: ฐานข้อมูล (แก้บั๊ก ลบแถวแล้วไม่หาย)
# ==========================================
with tab3:
    ...
    if not df_to_show.empty:
        ...
        if st.button("💾 บันทึกการเปลี่ยนแปลง (ลบ/แก้ไข)", type="primary"):
            try:
                # 1. หาแถวที่ถูกลบ: ดูว่า Index ไหนที่เคยมีในหน้าจอ แต่ตอนนี้หายไปจาก edited_df แล้ว
                ...
                # 3. อัปเดตข้อมูลที่เหลือ (การแก้ไขตัวเลข/ข้อความ)
                st.session_state.data.update(edited_df)
                
                # 4. บันทึกลงไฟล์
                save_data(st.session_state.data)
                
                st.success(f"✅ บันทึกสำเร็จ! (ลบ {len(deleted_indices)} รายการ, อัปเดตข้อมูลล่าสุด)")
                st.rerun()
