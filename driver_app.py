...
with tab2:
    ...
    if not df.empty:
        ...
        if not f_df.empty:
            ...
            # --- 🟢 วิเคราะห์ความคุ้มค่า (GP: เติมเงิน + ส่วนต่างตัดบัตร) ---
            with st.expander("💸 วิเคราะห์ความคุ้มค่า (GP & ค่าคอม)", expanded=True):
                # 1. ข้อมูลฝั่งจ่าย (ต้นทุนจากการเติมเครดิต เช่น Bolt/Maxim)
                app_expenses = exp_df[~exp_df['แอป'].isin(['ค่าใช้จ่าย', 'ระบบ'])].copy()
                app_expenses['แอป'] = app_expenses['แอป'].replace({'Grab Wallet': 'Grab'}) # รวมชื่อกันเหนียว
                ...
                if not app_incomes.empty:
                    ...
                    for app in all_apps:
                        ...
                        # B. ต้นทุนจากการเติมเครดิต (ดูจากตารางรายจ่าย)
                        deduct_from_expense = my_exp['หัก/จ่าย'].sum()
                        
                        # C. ต้นทุนจากการหักหน้างาน (คำนวณสด: หน้าแอป - รับจริง)
                        # เฉพาะกรณีที่ หน้าแอป > รับจริง (คือโดนหัก) 
                        # ถ้า หน้าแอป < รับจริง (ได้ทิป) จะไม่นับเป็นต้นทุน
                        # สูตร: (93 - 70) = 23
                        diff_series = my_inc['ยอดเต็ม/หน้าแอป'] - my_inc['คงเหลือ/สุทธิ']
                        deduct_from_income = diff_series[diff_series > 0].sum()
                        
                        # รวมต้นทุนทั้งหมด (เครดิต + ส่วนต่างตัดบัตร)
                        ...
                        if gross_income > 0:
                            ...
                            gp_data.append({
                                ...
                                "จากเติมเครดิต": deduct_from_expense,
                                "จากหักตัดบัตร": deduct_from_income
                            })
                        ...
                        with c_gp1: 
                            st.dataframe(
                                ...
                                column_config={
                                    ...
                                    "จากเติมเครดิต": st.column_config.NumberColumn(format="%.0f", help="ต้นทุนที่มาจากเมนู 'เติมเครดิต' (เช่น Bolt/Maxim)"),
                                    "จากหักตัดบัตร": st.column_config.NumberColumn(format="%.0f", help="ต้นทุนที่คำนวณจาก (ราคาหน้าแอป - รับจริง) เช่น Grab ตัดบัตร")
                                },
                                hide_index=True, 
