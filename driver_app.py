...
# ==========================================
# TAB 1: บันทึกงาน (กลับมาใช้รูปแบบเดิมเป๊ะๆ)
# ==========================================
with tab1:
    ...
    with col_form:
        ...
        # --- 1. รับงาน (รูปแบบเดิม: ราคาหน้าแอป + เงินรับจริงรวมทิป) ---
        if entry_type == "🚗 รับงานขับรถ":
            ...
            with st.form(key="form_income", clear_on_submit=True):
                ...
                # แถว 2 (ช่องกรอกเงินแบบเดิม)
                c1, c2 = st.columns(2)
                with c1: 
                    # Placeholder 0.00 และ value=None เพื่อให้พิมพ์ง่าย
                    app_price = st.number_input("ราคาหน้าแอป", min_value=0.0, step=10.0, value=None, placeholder="0.00")
                with c2: 
                    # 🔴 นี่ครับ! กลับมาเป็น 'เงินรับจริง (รวมทิป)' ตามเดิมแล้ว
                ...
                if submitted:
                    ...
                    if price_val > 0 or real_val > 0:
                        # Logic: ถ้าช่องรับจริงว่าง ให้ถือว่าเท่ากับราคาหน้าแอป
                        if real_val == 0: real_val = price_val 
                        
                        # คำนวณทิป (ส่วนต่าง)
                        tip = max(0, real_val - price_val)
                        
                        # รายได้ = เงินที่รับจริง (ไม่หัก GP ตามที่คุณขอ)
                        ...
                        if pay_method == "💵 เงินสด/โอน":
                            ...
                            cash_in_hand = 0.0 # ตัดบัตร เงินเข้าแอป
...
# ==========================================
# TAB 2: สรุปผล (จัดเต็มเหมือนเดิม: ประวัติ + กราฟ)
# ==========================================
with tab2:
    ...
    if not df.empty:
        ...
        if not filtered_df.empty:
            ...
            m1.metric("💰 กำไรสุทธิ (หักเติมเงินแล้ว)", f"{net:,.0f}", help="รายได้งาน - (น้ำมัน + เติมเครดิต + อื่นๆ)")
