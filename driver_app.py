...
# ==========================================
# TAB 1: บันทึกงาน (UI ใหม่: ซ่อนกล่อง Shift + แยกประเภทรายจ่าย)
# ==========================================
with tab1:
    # --- ฟังก์ชันช่วย ---
    def get_last_odom():
    ...
    # --- ส่วนที่ 2: จัดการกะงาน (แบบซ่อน: Expander) ---
    current_status = get_work_status()
    last_odom_val = get_last_odom()

    # 🟢 เลือกโชว์หัวข้อตามสถานะ
    ...
    # ใช้ st.expander เพื่อซ่อนกล่องใหญ่ๆ ไว้
    ...
    # 2. เติมพลังงาน (แก้ให้บันทึกชื่อรายการชัดเจนขึ้น เพื่ออนาคต)
    with sub_tab2:
        with st.form(key="form_energy", clear_on_submit=True):
            ...
            if st.form_submit_button("บันทึก", type="primary", use_container_width=True):
                if cost:
                    # 🟢 แก้ไข: บันทึก e_type ลงใน 'รายการ' เลย (แทนที่จะเป็น 'ค่าน้ำมัน/ไฟ')
                    # แต่ถ้าอยากให้โค้ดเก่าทำงานได้ราบรื่น ใส่ e_type นำหน้า note ไว้ด้วยก็ได้
                    new_row = {
                        'วันที่': get_thai_date(), 'เวลา': get_thai_time().strftime("%H:%M"),
                        'แอป': 'ค่าใช้จ่าย', 'หมวดหมู่': 'รายจ่าย', 
                        'รายการ': e_type, # บันทึกชื่อเต็มๆ เลย เช่น "⛽ น้ำมัน"
...
# ==========================================
# TAB 2: สรุปผล (Solution: แยกประเภทรายจ่ายจากข้อมูลเก่าได้)
# ==========================================
import calendar
import plotly.express as px
...
with tab2:
    ...
    if not df.empty:
        ...
        if not f_df.empty:
            ...
            with col_g4:
                # 🟢 Logic ใหม่: แยกประเภทรายจ่าย (ทั้งของเก่าและใหม่)
                if not exp_df.empty:
                    def categorize_expense(row):
                        item = row['รายการ']
                        note = str(row['หมายเหตุ'])
                        
                        # ถ้าเป็นข้อมูลเก่า Item จะเป็น 'ค่าน้ำมัน/ไฟ' -> ต้องไปแกะจาก Note
                        if item == 'ค่าน้ำมัน/ไฟ':
                            if 'น้ำมัน' in note: return '⛽ น้ำมัน'
                            if 'ชาร์จ' in note: return '⚡ ชาร์จไฟ'
                            return '⛽ พลังงาน'
                        return item # ถ้าเป็นข้อมูลใหม่ ใช้ค่าที่บันทึกได้เลย

                    # สร้าง Column ใหม่ชั่วคราวเพื่อทำกราฟ
                    exp_df_plot = exp_df.copy()
                    exp_df_plot['ประเภทกราฟ'] = exp_df_plot.apply(categorize_expense, axis=1)
                    
                    exp_sum = exp_df_plot.groupby('ประเภทกราฟ')['หัก/จ่าย'].sum().reset_index()
                    fig_exp = px.bar(exp_sum, x='ประเภทกราฟ', y='หัก/จ่าย', title="💸 รายจ่ายแยกประเภท", color='ประเภทกราฟ', text_auto=True)
