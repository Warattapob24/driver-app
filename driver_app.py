...
# ==========================================
# TAB 2: สรุปผล (Super Detail: แยกรายจ่ายละเอียดทุกเม็ด)
# ==========================================
import calendar
import plotly.express as px
...
with tab2:
    ...
    if not df.empty:
        ...
        if not f_df.empty:
            ...
            with col_g4:
                # 🟢 Logic ใหม่: เจาะลึกรายจ่ายทุกเม็ด!
                if not exp_df.empty:
                    def detailed_expense_name(row):
                        ...
                        # 1. กลุ่มเติมเครดิต -> ให้โชว์ชื่อแอป
                        if 'เติมเครดิต' in item:
                            return f"💳 เติม {app_name}"
                        
                        # 2. กลุ่มพลังงาน -> แยกน้ำมัน/ไฟ
                        if 'น้ำมัน' in item or 'ไฟ' in item:
                            ...
                            return "⛽ พลังงาน" # กรณีระบุไม่ได้
                        
                        # 3. กลุ่มทั่วไป -> ให้เอา 'หมายเหตุ' มาโชว์ (เช่น ปะยาง, ข้าว)
                        if 'ทั่วไป' in item:
                            if note and note.lower() != 'nan' and note != '':
                                return f"🛠️ {note}"
                            return "🛠️ จ่ายทั่วไป"
                        
                        return item # กรณีอื่นๆ ใช้ชื่อเดิม
                    ...
                    # ใช้กราฟแนวนอน (Horizontal Bar) เพื่อให้อ่านชื่อยาวๆ ง่ายขึ้น
                    fig_exp = px.bar(
                        exp_sum, 
                        x='หัก/จ่าย', 
                        y='ชื่อรายการกราฟ', 
