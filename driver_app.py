...
with st.sidebar:
    st.title("⚙️ ตั้งค่า")
    # แสดงเวลาปัจจุบันให้ดูเพื่อความมั่นใจ
    current_time = get_thai_time().strftime("%H:%M")
    st.info(f"🕒 เวลาไทยขณะนี้: {current_time}")
    ...
    with col_form:
        ...
        if entry_type == "🕒 เริ่มงาน/เลิกงาน (เลขไมล์)":
            ...
            if st.button("บันทึกเลขไมล์ 💾", type="primary", use_container_width=True):
                if odometer > 0:
                    new_row = {
                        'วันที่': get_thai_date(),  # ใช้เวลาไทย
                        'เวลา': get_thai_time().strftime("%H:%M"), # ใช้เวลาไทย
                    ...
                    save_data(st.session_state.data)
                    st.toast(f"บันทึก {shift_type} เวลา {new_row['เวลา']} เรียบร้อย!")
        ...
        # --- 2. รับงาน ---
            ...
            with st.expander("ใส่ระยะทาง (ไม่บังคับ)"):
