...
# --- 2. SETTINGS & MEMORY ---
def load_settings():
    default_settings = {"ev_rate": 40.0} # เหลือแค่ค่าไฟ
...
def load_and_clean_data():
    try:
        ...
        col_map = {
            ...
            'Amount_Gross': 'ยอดเต็ม', 'Deduction': 'หัก/จ่าย',
        ...
        # แปลงตัวเลข
        num_cols = ['ยอดเต็ม', 'หัก/จ่าย', 'ทิป', 'คงเหลือ/สุทธิ', 'เงินสดเข้าตัว', 'เลขไมล์']
        for col in num_cols:
        ...
        return pd.DataFrame(columns=[
            ...
            'ยอดเต็ม', 'หัก/จ่าย', 'ทิป', 'คงเหลือ/สุทธิ', 
...
# --- 4. SIDEBAR (ตัดการตั้งค่า GP ออกแล้ว) ---
with st.sidebar:
    st.title("⚙️ ตั้งค่า")
    st.caption(f"เวลา: {get_thai_time().strftime('%H:%M')}")
    
    current_settings = load_settings()
    new_ev_rate = st.number_input("ค่าไฟชาร์จบ้าน (เหมาจ่าย)", value=float(current_settings.get("ev_rate", 40.0)), step=5.0)
...
# ==========================================
# TAB 1: บันทึกงาน (Simple Logic: No GP Calculation)
# ==========================================
with tab1:
    ...
    with col_form:
        ...
        # --- 1. รับงาน (บันทึกยอดเต็ม ไม่หักอะไรทั้งนั้น) ---
        if entry_type == "🚗 รับงานขับรถ":
            ...
            with st.form(key="form_income", clear_on_submit=True):
                ...
                with c_pay:
                    pay_method = st.selectbox("ลูกค้าจ่ายทางไหน?", ["💵 เงินสด/โอน", "💳 ตัดบัตร/แอป"])

                c1, c2 = st.columns(2)
                with c1: 
                    app_price = st.number_input("ราคาหน้าแอป", min_value=0.0, step=10.0, value=None, placeholder="0.00")
                with c2: 
                    # ถ้ามีทิป ให้ใส่เพิ่มตรงนี้
                    tip_amount = st.number_input("ทิป (ถ้ามี)", min_value=0.0, step=10.0, value=None, placeholder="0.00")
                ...
                if submitted:
                    ...
                    tip_val = tip_amount if tip_amount is not None else 0.0
                    
                    if price_val > 0:
                        # LOGIC ใหม่: รายได้ = ราคา + ทิป (ไม่ต้องหักอะไรเลย)
                        total_income = price_val + tip_val
                        
                        # คำนวณเงินสดเข้าตัว (Cash Flow)
                        cash_in_hand = 0.0
                        if pay_method == "💵 เงินสด/โอน":
                            cash_in_hand = total_income
                        else:
                            cash_in_hand = 0.0 # ตัดบัตร เงินเข้าแอป เราไม่ได้จับเงินสด
                        ...
                        new_row = {
                            ...
                            'ยอดเต็ม': price_val, 'หัก/จ่าย': 0, 'ทิป': tip_val, 
                            'คงเหลือ/สุทธิ': total_income, # รายได้เต็มเม็ดเต็มหน่วย
                        ...
                        if cash_in_hand > 0: msg += f" (เข้ากระเป๋า {cash_in_hand:.0f})"
                        st.toast(msg)
                        st.rerun()
                    else:
                        st.warning("กรุณากรอกยอดเงิน")

        # --- 2. เติมเครดิต (นับเป็นรายจ่าย) ---
            ...
            with st.form(key="form_topup", clear_on_submit=True):
                ...
                if submitted:
                    ...
                    if cost_val > 0:
                        # LOGIC ใหม่: การเติมเงิน = รายจ่าย (Expense)
                        new_row = {
                            'วันที่': get_thai_date(), 'เวลา': get_thai_time().strftime("%H:%M"),
                            'แอป': sub_cat, 'หมวดหมู่': 'รายจ่าย', 'รายการ': 'เติมเครดิต', 'ช่องทางรับเงิน': 'จ่ายสด',
                            'ยอดเต็ม': 0, 'หัก/จ่าย': cost_val, 'ทิป': 0, 
                            'คงเหลือ/สุทธิ': -cost_val, # กำไรลดลงทันทีที่เติม
                            'เงินสดเข้าตัว': -cost_val, # เงินสดหายไปจากกระเป๋า
                            'เลขไมล์': 0, 'หมายเหตุ': 'Top-up'
                        }
                        st.session_state.data = pd.concat([st.session_state.data, pd.DataFrame([new_row])], ignore_index=True)
                        save_data(st.session_state.data)
                        st.toast(f"บันทึกค่าใช้จ่ายเติมเงิน {cost_val} บาทแล้ว")
                        st.rerun()
                    else: st.warning("กรุณากรอกจำนวนเงิน")

        # --- 3. น้ำมัน/ไฟ (เหมือนเดิม) ---
                ...
                if submitted:
                    ...
                    if odom_val > 0:
                        new_row = {
                            ...
                            'ยอดเต็ม': 0, 'หัก/จ่าย': 0, 'ทิป': 0, 'คงเหลือ/สุทธิ': 0, 'เงินสดเข้าตัว': 0,
...
# ==========================================
with tab2:
    st.markdown("### 📊 สรุปผล (ระบบ Cash Basis)")
    ...
    if not df.empty:
        ...
        if not filtered_df.empty:
            ...
            # 2. การเงิน (Cash Basis)
            # รายรับ = ยอดงานขับรถทั้งหมด (รวมทิป)
            total_income = filtered_df[filtered_df['หมวดหมู่'] == 'รายรับ']['คงเหลือ/สุทธิ'].sum()
            
            # รายจ่าย = ค่าน้ำมัน + เติมเครดิต + จ่ายอื่นๆ
            # (สังเกต: เราเอา 'หัก/จ่าย' มารวมกัน ซึ่งรวมค่าเติมเครดิตด้วย)
            total_expense = filtered_df[filtered_df['หมวดหมู่'] == 'รายจ่าย']['หัก/จ่าย'].sum()
            ...
            # กล่องเงินสด (สำคัญสุด)
            st.container(border=True).markdown(f"### 💵 เงินสดที่จับต้องได้จริง: {cash_in_hand:,.0f} บาท")
            
            m1, m2, m3, m4 = st.columns(4)
            m1.metric("💰 กำไรสุทธิ (หักเติมเงินแล้ว)", f"{net_profit:,.0f}", help="รายได้งาน - (น้ำมัน + เติมเครดิต + อื่นๆ)")
            m2.metric("💸 รายจ่ายรวม", f"{total_expense:,.0f}", help="รวมค่าน้ำมัน และการเติมเงินเข้าแอป")
            ...
            # สถิติ
            s1, s2 = st.columns(2)
            if total_km > 0: s1.metric("📊 รายได้/กม.", f"{total_income/total_km:.1f} บ.")
            if total_hours > 0: s2.metric("💵 รายได้/ชม.", f"{total_income/total_hours:.0f} บ.")
            ...
            with c1:
                ...
                if not inc_df.empty: st.plotly_chart(px.bar(inc_df.groupby('แอป')['คงเหลือ/สุทธิ'].sum().reset_index(), x='แอป', y='คงเหลือ/สุทธิ', color='แอป', text_auto=True, title="รายได้แยกแอป"), use_container_width=True)
            with c2:
                # รายจ่ายแยกประเภท
                exp_df = filtered_df[filtered_df['หมวดหมู่'] == 'รายจ่าย']
                if not exp_df.empty: st.plotly_chart(px.pie(exp_df, values='หัก/จ่าย', names='รายการ', title="สัดส่วนรายจ่าย (น้ำมัน vs เติมแอป)", hole=0.4), use_container_width=True)
