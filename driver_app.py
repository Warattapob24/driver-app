import streamlit as st
import pandas as pd
import datetime
import plotly.express as px

# --- 1. การตั้งค่าและเตรียมระบบ ---
st.set_page_config(page_title="ระบบบันทึกรายได้คนขับ", page_icon="🇹🇭", layout="wide")

# ชื่อไฟล์เก็บข้อมูล
DATA_FILE = "driver_data.csv"

# ฟังก์ชันโหลดข้อมูล (พร้อมระบบแปลงภาษาอังกฤษ -> ไทย อัตโนมัติ)
def load_data():
    try:
        df = pd.read_csv(DATA_FILE)
        
        # --- AUTO-MIGRATE: แปลงหัวตารางจากอังกฤษเป็นไทย (ถ้ามี) ---
        eng_to_thai = {
            'Date': 'วันที่',
            'Time': 'เวลา',
            'Platform': 'แอป',
            'Category': 'หมวดหมู่',
            'SubCategory': 'รายการ',
            'Amount_Gross': 'ยอดเต็ม/หน้าแอป',
            'Deduction': 'หัก/จ่าย',
            'Tip': 'ทิป',
            'Net_Income': 'คงเหลือ/สุทธิ',
            'Distance_Km': 'ระยะทาง(กม.)',
            'Note': 'หมายเหตุ'
        }
        # เปลี่ยนชื่อคอลัมน์ถ้ามันยังเป็นภาษาอังกฤษ
        df.rename(columns=eng_to_thai, inplace=True)
        
        # --- ตรวจสอบว่ามีคอลัมน์ครบไหม (ถ้าไม่มีให้เติม) ---
        required_columns = [
            'วันที่', 'เวลา', 'แอป', 'หมวดหมู่', 'รายการ', 
            'ยอดเต็ม/หน้าแอป', 'หัก/จ่าย', 'ทิป', 'คงเหลือ/สุทธิ', 'ระยะทาง(กม.)', 'หมายเหตุ'
        ]
        
        for col in required_columns:
            if col not in df.columns:
                if col in ['ยอดเต็ม/หน้าแอป', 'หัก/จ่าย', 'ทิป', 'คงเหลือ/สุทธิ', 'ระยะทาง(กม.)']:
                    df[col] = 0.0
                else:
                    df[col] = ""

        # แปลงคอลัมน์วันที่ให้เป็นรูปแบบวันที่จริงๆ
        df['วันที่'] = pd.to_datetime(df['วันที่']).dt.date
        return df[required_columns] # จัดเรียงลำดับให้สวยงาม
        
    except FileNotFoundError:
        # ถ้าไม่มีไฟล์เลย ให้สร้างตารางเปล่าภาษาไทย
        return pd.DataFrame(columns=[
            'วันที่', 'เวลา', 'แอป', 'หมวดหมู่', 'รายการ', 
            'ยอดเต็ม/หน้าแอป', 'หัก/จ่าย', 'ทิป', 'คงเหลือ/สุทธิ', 'ระยะทาง(กม.)', 'หมายเหตุ'
        ])

def save_data(df):
    df.to_csv(DATA_FILE, index=False)

# โหลดข้อมูลเข้าสู่ระบบ
if 'data' not in st.session_state:
    st.session_state.data = load_data()

# --- 2. แถบเมนูด้านข้าง (Sidebar) ---
with st.sidebar:
    st.title("⚙️ ตั้งค่าระบบ")
    st.write("ตั้งค่าตัวช่วยคำนวณ")
    maxim_comm_rate = st.slider("Maxim หักคอมมิชชั่น (%)", 0, 30, 15) / 100
    ev_home_rate = st.number_input("ค่าไฟชาร์จบ้าน (เหมาจ่าย/ครั้ง)", value=40, step=5)
    
    st.divider()
    st.info("💡 คำแนะนำ: คุณสามารถแก้ไขข้อมูลย้อนหลังได้ที่เมนู 'ฐานข้อมูล'")

# --- 3. หน้าจอหลัก (Main App) ---
st.title("🇹🇭 ระบบบันทึกรายได้ (Driver Pro)")

# สร้างแท็บ 3 หน้า
tab1, tab2, tab3 = st.tabs(["📝 บันทึกงาน", "📊 สรุปผลกำไร", "🗂️ ฐานข้อมูล"])

# ==========================================
# TAB 1: บันทึกงาน (Entry)
# ==========================================
with tab1:
    col_type, col_form = st.columns([1, 2])
    
    with col_type:
        st.subheader("เลือกรายการ")
        entry_type = st.radio(
            "ประเภทรายการ",
            ["🚗 รับงานขับรถ", "⛽ เติมน้ำมัน/ชาร์จไฟ", "💳 เติมเครดิตแอป", "🛠️ จ่ายอื่นๆ"],
            captions=["รายได้จากการวิ่งงาน", "ค่าน้ำมัน/ค่าไฟ", "เติมเงินเข้ากระเป๋าแอป", "ค่ากิน/ค่าซ่อม/อื่นๆ"]
        )

    with col_form:
        st.container(border=True)
        # --- ฟอร์ม 1: รับงาน ---
        if entry_type == "🚗 รับงานขับรถ":
            st.markdown("#### 📝 บันทึกรายได้")
            # ชื่อแอปยังคงเป็นภาษาอังกฤษตามที่ขอ
            platform = st.selectbox("เลือกแอป", ["Grab", "Bolt", "Line Man", "Maxim", "Robinhood", "Win", "งานนอก"])
            
            c1, c2 = st.columns(2)
            with c1: 
                app_price = st.number_input("ราคาหน้าแอป (บาท)", min_value=0.0, step=10.0)
            with c2: 
                real_receive = st.number_input("เงินที่รับจริง (รวมทิป)", min_value=0.0, value=app_price, step=10.0)
            
            distance = st.number_input("ระยะทางงานนี้ (กม.)", min_value=0.0, step=1.0)
            note = st.text_input("หมายเหตุ (ถ้ามี)")
            
            if st.button("บันทึกรายได้ ✅", type="primary", use_container_width=True):
                if app_price > 0:
                    deduction = 0
                    tip = max(0, real_receive - app_price)
                    
                    # Logic คำนวณ (Maxim/งานนอก คิด % หัก)
                    if platform in ["Maxim", "งานนอก"]:
                        deduction = app_price * maxim_comm_rate
                        net_income = app_price - deduction + tip
                    else:
                        # Grab/Bolt ถือว่ายอด Net แล้ว
                        net_income = app_price + tip 

                    new_row = {
                        'วันที่': datetime.date.today(),
                        'เวลา': datetime.datetime.now().strftime("%H:%M"),
                        'แอป': platform,
                        'หมวดหมู่': 'รายรับ',
                        'รายการ': 'ค่าโดยสาร',
                        'ยอดเต็ม/หน้าแอป': app_price,
                        'หัก/จ่าย': deduction,
                        'ทิป': tip,
                        'คงเหลือ/สุทธิ': net_income,
                        'ระยะทาง(กม.)': distance,
                        'หมายเหตุ': note
                    }
                    st.session_state.data = pd.concat([st.session_state.data, pd.DataFrame([new_row])], ignore_index=True)
                    save_data(st.session_state.data)
                    st.toast(f"บันทึกสำเร็จ! เข้ากระเป๋า {net_income:.0f} บาท")

        # --- ฟอร์ม 2: พลังงาน ---
        elif entry_type == "⛽ เติมน้ำมัน/ชาร์จไฟ":
            st.markdown("#### ⚡ บันทึกต้นทุนพลังงาน")
            e_type = st.radio("ประเภท", ["⛽ น้ำมัน", "⚡ ชาร์จบ้าน (เหมา)", "🔌 ชาร์จสถานี"], horizontal=True)
            
            cost = st.number_input("จำนวนเงินที่จ่าย (บาท)", value=(ev_home_rate if "บ้าน" in e_type else 0.0))
            note = st.text_input("สถานที่ / ปั๊ม")
            
            if st.button("บันทึกค่าพลังงาน 💸", type="primary", use_container_width=True):
                if cost > 0:
                    new_row = {
                        'วันที่': datetime.date.today(),
                        'เวลา': datetime.datetime.now().strftime("%H:%M"),
                        'แอป': 'ค่าใช้จ่าย',
                        'หมวดหมู่': 'รายจ่าย',
                        'รายการ': 'ค่าน้ำมัน/ไฟ',
                        'ยอดเต็ม/หน้าแอป': 0,
                        'หัก/จ่าย': cost,
                        'ทิป': 0,
                        'คงเหลือ/สุทธิ': -cost, # เป็นค่าลบ
                        'ระยะทาง(กม.)': 0,
                        'หมายเหตุ': f"{e_type} - {note}"
                    }
                    st.session_state.data = pd.concat([st.session_state.data, pd.DataFrame([new_row])], ignore_index=True)
                    save_data(st.session_state.data)
                    st.toast("บันทึกเรียบร้อย")

        # --- ฟอร์ม 3: เติมเครดิต ---
        elif entry_type == "💳 เติมเครดิตแอป":
            st.markdown("#### 💳 เติมเงินเข้ากระเป๋าแอป")
            platform = st.selectbox("เติมเข้าแอปไหน?", ["Grab Wallet", "Bolt Balance", "Maxim", "Line Man Credit"])
            amount = st.number_input("จำนวนเงิน (บาท)", min_value=0.0, step=100.0)
            
            if st.button("บันทึกการเติมเงิน", type="primary", use_container_width=True):
                new_row = {
                    'วันที่': datetime.date.today(),
                    'เวลา': datetime.datetime.now().strftime("%H:%M"),
                    'แอป': platform,
                    'หมวดหมู่': 'รายจ่าย',
                    'รายการ': 'เติมเครดิต',
                    'ยอดเต็ม/หน้าแอป': 0,
                    'หัก/จ่าย': amount,
                    'ทิป': 0,
                    'คงเหลือ/สุทธิ': -amount,
                    'ระยะทาง(กม.)': 0,
                    'หมายเหตุ': "เติมเงินงาน"
                }
                st.session_state.data = pd.concat([st.session_state.data, pd.DataFrame([new_row])], ignore_index=True)
                save_data(st.session_state.data)
                st.toast("บันทึกเรียบร้อย")

        # --- ฟอร์ม 4: อื่นๆ ---
        elif entry_type == "🛠️ จ่ายอื่นๆ":
             st.markdown("#### 🛠️ ค่าใช้จ่ายทั่วไป")
             cost = st.number_input("จำนวนเงิน (บาท)", min_value=0.0)
             note = st.text_input("รายการ (เช่น ปะยาง, ข้าว, กาแฟ)")
             if st.button("บันทึก", type="primary", use_container_width=True):
                 new_row = {
                        'วันที่': datetime.date.today(),
                        'เวลา': datetime.datetime.now().strftime("%H:%M"),
                        'แอป': 'ค่าใช้จ่าย',
                        'หมวดหมู่': 'รายจ่าย',
                        'รายการ': 'ทั่วไป',
                        'ยอดเต็ม/หน้าแอป': 0,
                        'หัก/จ่าย': cost,
                        'ทิป': 0,
                        'คงเหลือ/สุทธิ': -cost,
                        'ระยะทาง(กม.)': 0,
                        'หมายเหตุ': note
                    }
                 st.session_state.data = pd.concat([st.session_state.data, pd.DataFrame([new_row])], ignore_index=True)
                 save_data(st.session_state.data)
                 st.toast("บันทึกเรียบร้อย")

# ==========================================
# TAB 2: สรุปผล (Analytics)
# ==========================================
with tab2:
    df = st.session_state.data
    
    if not df.empty:
        # แยกข้อมูล
        income_df = df[df['หมวดหมู่'] == 'รายรับ']
        expense_df = df[df['หมวดหมู่'] == 'รายจ่าย']
        
        # คำนวณยอดต่างๆ
        total_income = income_df['คงเหลือ/สุทธิ'].sum()
        total_tips = income_df['ทิป'].sum()
        
        # แยกประเภทค่าใช้จ่าย
        fuel_cost = expense_df[expense_df['รายการ'] == 'ค่าน้ำมัน/ไฟ']['หัก/จ่าย'].sum()
        topup_cost = expense_df[expense_df['รายการ'] == 'เติมเครดิต']['หัก/จ่าย'].sum()
        other_cost = expense_df[expense_df['รายการ'] == 'ทั่วไป']['หัก/จ่าย'].sum()
        
        total_km = income_df['ระยะทาง(กม.)'].sum()
        
        # กำไรสุทธิเข้ากระเป๋า (ไม่หักเติมเครดิต เพราะถือเป็นเงินหมุนเวียน)
        net_profit_pocket = total_income - fuel_cost - other_cost
        
        # แสดงผลตัวเลข (Metrics)
        m1, m2, m3, m4 = st.columns(4)
        m1.metric("💰 กำไรเข้ากระเป๋าจริง", f"{net_profit_pocket:,.0f} บ.", delta="หักค่าน้ำมัน/กินแล้ว")
        m2.metric("⛽ จ่ายค่าน้ำมัน/ไฟ", f"{fuel_cost:,.0f} บ.")
        m3.metric("🛣️ วิ่งงานไปแล้ว", f"{total_km:,.0f} กม.")
        
        if total_km > 0:
            income_per_km = total_income / total_km
            cost_per_km = fuel_cost / total_km
            m4.metric("📊 รายได้เฉลี่ย/กม.", f"{income_per_km:.2f} บ.", f"ต้นทุน {cost_per_km:.2f} บ./กม.")
        else:
            m4.metric("📊 รายได้เฉลี่ย/กม.", "0.00", "รอข้อมูลระยะทาง")

        st.markdown("---")

        # กราฟแสดงผล
        c1, c2 = st.columns(2)
        
        with c1:
            st.subheader("🏆 แอปไหนทำเงินดีสุด?")
            if not income_df.empty:
                plat_sum = income_df.groupby('แอป')['คงเหลือ/สุทธิ'].sum().reset_index()
                fig_bar = px.bar(plat_sum, x='แอป', y='คงเหลือ/สุทธิ', color='แอป', text_auto=True, title="รายได้แยกตามแอป")
                st.plotly_chart(fig_bar, use_container_width=True)
            else:
                st.info("ยังไม่มีข้อมูลรายได้")
                
        with c2:
            st.subheader("💸 รายจ่ายแยกประเภท")
            if not expense_df.empty:
                exp_sum = expense_df.groupby('รายการ')['หัก/จ่าย'].sum().reset_index()
                fig_pie = px.pie(exp_sum, values='หัก/จ่าย', names='รายการ', hole=0.4, title="สัดส่วนรายจ่าย")
                st.plotly_chart(fig_pie, use_container_width=True)
            else:
                st.info("ยังไม่มีข้อมูลค่าใช้จ่าย")

        # Heatmap ช่วงเวลา
        st.subheader("🔥 ช่วงเวลาทำเงิน (วิ่งตอนไหนดี?)")
        if not income_df.empty:
            income_df['ชั่วโมง'] = pd.to_datetime(income_df['เวลา'], format='%H:%M').dt.hour
            fig_hist = px.histogram(income_df, x='ชั่วโมง', y='คงเหลือ/สุทธิ', nbins=24, 
                                  title="รายได้รวม แยกตามช่วงเวลาของวัน (0-23 น.)", 
                                  color_discrete_sequence=['#FFD700'],
                                  labels={'ชั่วโมง':'เวลา (นาฬิกา)', 'คงเหลือ/สุทธิ':'รายได้ (บาท)'})
            fig_hist.update_layout(bargap=0.1)
            st.plotly_chart(fig_hist, use_container_width=True)
        
    else:
        st.info("👋 ยินดีต้อนรับ! เริ่มบันทึกข้อมูลหน้าแรกได้เลยครับ")

# ==========================================
# TAB 3: ฐานข้อมูล (Database)
# ==========================================
with tab3:
    st.subheader("🗂️ ฐานข้อมูล (แก้ไข/ลบ)")
    st.info("📝 แก้ไขข้อมูลในตารางนี้ได้เลย (ภาษาไทย) ระบบจะบันทึกให้อัตโนมัติ")
    
    if not st.session_state.data.empty:
        edited_df = st.data_editor(
            st.session_state.data,
            num_rows="dynamic",
            use_container_width=True,
            key="editor"
        )
        
        if st.button("💾 บันทึกการแก้ไขลงไฟล์", type="primary"):
            st.session_state.data = edited_df
            save_data(edited_df)
            st.success("บันทึกข้อมูลเรียบร้อยแล้ว!")
    else:
        st.write("ไม่มีข้อมูลให้แก้ไข")
