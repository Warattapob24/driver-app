...
with tab2:
    ...
    if not df.empty:
        ...
        if not f_df.empty:
            ...
            # --- 🟢 วิเคราะห์ความคุ้มค่า (GP: เติมเงิน + คำนวณสดจากส่วนต่างตัดบัตร) ---
            with st.expander("💸 วิเคราะห์ความคุ้มค่า (GP & ค่าคอม)", expanded=True):
                ...
                app_expenses['แอป'] = app_expenses['แอป'].replace({'Grab Wallet': 'Grab'}) # แก้ชื่อให้ตรงกัน
                
                # 2. ข้อมูลฝั่งรับ (งานที่ขับ)
                ...
                if not app_incomes.empty:
                    ...
                    for app in all_apps:
                        # กรองข้อมูลเฉพาะแอปนี้
                        this_app_inc = app_incomes[app_incomes['แอป'] == app]
                        this_app_exp = app_expenses[app_expenses['แอป'] == app]

                        # A. ยอดงานทั้งหมด (Gross)
                        gross_income = this_app_inc['ยอดเต็ม/หน้าแอป'].sum()
                        
                        # B. ต้นทุนจาก "การเติมเครดิต" (ดูจากรายจ่าย)
                        deduct_from_topup = this_app_exp['หัก/จ่าย'].sum()
                        
                        # C. ต้นทุนจาก "ส่วนต่างงานตัดบัตร" (คำนวณสด!)
                        # Logic: ถ้ารับงานแบบ "ตัดบัตร/แอป" ให้เอา (หน้าแอป - รับจริง) มาเป็นค่าคอม
                        # ต้องกรองเฉพาะที่ หน้าแอป > รับจริง (เพื่อกันกรณีทิป หรือข้อมูลผิดพลาด)
                        def calc_card_deduction(row):
                            is_card = 'บัตร' in str(row['ช่องทางรับเงิน']) or 'แอป' in str(row['ช่องทางรับเงิน'])
                            diff = row['ยอดเต็ม/หน้าแอป'] - row['คงเหลือ/สุทธิ']
                            # ถ้าเป็นตัดบัตร และ ยอดหน้าแอปมากกว่าที่ได้จริง -> ส่วนต่างคือค่าคอม
                            if is_card and diff > 0:
                                return diff
                            # กรณีอื่นๆ ให้เชื่อตามช่อง 'หัก/จ่าย' เดิมที่บันทึกมา (เผื่อมีการบันทึกไว้)
                            return row['หัก/จ่าย']

                        # คำนวณใหม่ทีละแถว แล้วรวมผล
                        deduct_from_card = this_app_inc.apply(calc_card_deduction, axis=1).sum()
                        
                        # รวมต้นทุนทั้งหมด (เติมเงิน + โดนหักหน้างาน)
                        total_deduction = deduct_from_topup + deduct_from_card
                        ...
                        if gross_income > 0:
                            ...
                            gp_data.append({
                                ...
                                "หักหน้างาน (คำนวณสด)": deduct_from_card
                            })
                    ...
                    if gp_data:
                        ...
                        with c_gp1: 
                            st.dataframe(
                                ...
                                column_config={
                                    ...
                                    "หักหน้างาน (คำนวณสด)": st.column_config.NumberColumn(format="%.0f", help="คำนวณจาก (ราคาหน้าแอป - รับจริง) เฉพาะงานตัดบัตร")
                                },
                                hide_index=True, 
                        ...
                        with c_gp2:
                             # กราฟแสดงผล
                            fig = px.bar(gp_df, x='GP (%)', y='แอป', orientation='h', title="📉 ต้นทุนแอป (รวมเติมเงิน + โดนหักหน้างาน)", text_auto='.1f', color='GP (%)', color_continuous_scale='Reds')
