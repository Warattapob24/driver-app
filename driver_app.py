    ...
    with col_form:
        ...
        if entry_type == "🚗 รับงานขับรถ":
            ...
            with st.form(key="form_income", clear_on_submit=True):
                ...
                with c2: real_receive = st.number_input("เงินที่รับจริง (รวมทิป)", min_value=0.0, step=10.0, value=None, placeholder="เท่าหน้าแอป")
                ...
                if submitted:
                    ...
                    if price_val > 0 or real_val > 0:
                        ...
                        new_row = {
                            ...
                            'คงเหลือ/สุทธิ': total_income, 'เงินสดเข้าตัว': cash_in_hand, 'เลขไมล์': 0, 'หมายเหตุ': note
                        }
...
# ==========================================
# TAB 2: สรุปผล (เอาฟีเจอร์กราฟกลับมาครบ!)
# ==========================================
with tab2:
    ...
    if not df.empty:
        ...
        if not filtered_df.empty:
            ...
            m1.metric("💰 กำไรสุทธิ (หักค่าใช้จ่าย)", f"{net:,.0f}")
            ...
            with row1_c1:
                ...
                else: st.info("ไม่มีรายรับ")
            
            with row1_c2:
                # ช่วงเวลาทำเงิน (Histogram) - กู้คืนมาแล้ว!
                if not inc_df.empty:
                    inc_df['Hour'] = pd.to_datetime(inc_df['เวลา'], format='%H:%M').dt.hour
                    st.plotly_chart(px.histogram(inc_df, x='Hour', y='คงเหลือ/สุทธิ', nbins=24, title="🔥 ช่วงเวลาทำเงิน (0-24น.)", color_discrete_sequence=['#FF4B4B']), use_container_width=True)
                else: st.info("ไม่มีข้อมูลเวลา")

            row2_c1, row2_c2 = st.columns(2)
            with row2_c1:
                # สัดส่วนรายจ่าย (Pie Chart)
                if not exp_df.empty: 
                    st.plotly_chart(px.pie(exp_df, values='หัก/จ่าย', names='รายการ', title="💸 สัดส่วนค่าใช้จ่าย", hole=0.4), use_container_width=True)
            
            with row2_c2:
                # สัดส่วนช่องทางรับเงิน (Pie Chart)
                if not inc_df.empty:
                    st.plotly_chart(px.pie(inc_df, names='ช่องทางรับเงิน', values='คงเหลือ/สุทธิ', title="💳 ช่องทางรับเงิน (สด/แอป)", hole=0.4), use_container_width=True)
