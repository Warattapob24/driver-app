...
# ==========================================
# TAB 2: สรุปผล (GP Calculation Fixed)
# ==========================================
import calendar
...
with tab2:
    ...
    if not df.empty:
        ...
        if not f_df.empty:
            ...
            # --- 🟢 วิเคราะห์ความคุ้มค่า (GP: เติมเงิน + ตัดบัตร) ---
            with st.expander("💸 วิเคราะห์ความคุ้มค่า (GP & ค่าคอม)", expanded=True):
                ...
                app_expenses['แอป'] = app_expenses['แอป'].replace({'Grab Wallet': 'Grab'}) # รวมชื่อ
                
                # 2. ข้อมูลฝั่งรับ (โดนหักหน้างาน สำหรับตัดบัตร)
                ...
                if not app_incomes.empty:
                    ...
                    for app in all_apps:
                        ...
                        # A: หักจากการเติมเครดิต (ดูจากรายจ่าย)
                        deduct_from_expense = app_expenses[app_expenses['แอป'] == app]['หัก/จ่าย'].sum()
                        
                        # B: หักหน้างาน/ตัดบัตร (ดูจากรายรับ)
                        ...
                        with c_gp1: 
                            st.dataframe(
                                ...
                                column_config={
                                    ...
                                    "จากหักตัดบัตร": st.column_config.NumberColumn(format="%.0f", help="มาจากส่วนต่าง ราคาหน้าแอป-รับจริง")
                                },
                                hide_index=True, 
